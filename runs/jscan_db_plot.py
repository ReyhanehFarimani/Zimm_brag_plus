...
red points: MC N = 200 with the six-argument helix-helix table + registry twist term, finished runs
only, errors = max(SEM over seeds, combined 20-block error).  Dotted line: J*(N = 200) of the exact
...
E = {N: {k: np.zeros(len(Jg)) for k in ("th", "chi_th", "am", "U4")} for N in NS}; CACHE = "exact_cache_db.npz"
...
for log in sorted(glob.glob("logs/J*_db_s*.log")):
    if "summary" not in open(log).read():
        continue
    b = os.path.basename(log)[:-4]; J = float(re.match(r"J([\d.]+)_db", b).group(1))
    o = read_obs(os.path.join("out", b + "_obs.dat")); m = (o["n_R"] - o["n_L"]) / N0
    runs.setdefault(J, []).append(dict(th=o["helicity"].mean(), am=np.abs(m).mean(), rg2=o["Rg2"].mean(), enb=o["E_nb"].mean(),
...
Js = sorted(runs); nfin = sum(len(v) for v in runs.values()); ntot = len(glob.glob("inputs/J*.dat"))
...
for c, (k, yl) in enumerate(TOP):
    x = ax[0, c]
    for N in NS:
        x.plot(Jg, E[N][k], color=CN[N], lw=1.8, label=f"exact 1D, N = {N}" if c == 0 else None)
    pts = [(J,) + cell(J, k)[:2] for J in Js]
    if pts:
        X, V, Er = map(np.array, zip(*pts))
        x.errorbar(X, V, yerr=Er, fmt="o", ms=5.5, mfc=RED, mec="white", mew=0.6, ecolor=RED, elinewidth=0.9, zorder=5,
                   label="MC N = 200, table + twist" if c == 0 else None)
    x.axvline(JSTAR[GEOM], color=MUTED, lw=0.8, ls=":")
...
ax[0, 0].legend(loc="lower right", handlelength=1.5)
for c, (k, yl) in enumerate(BOT):
    x = ax[1, c]
    pts = [(J,) + cell(J, k)[:2] for J in Js if np.isfinite(cell(J, k)[0])]
    if pts:
        X, V, Er = map(np.array, zip(*pts))
        x.errorbar(X, V, yerr=Er, fmt="o-", ms=5.5, lw=1.0, mfc=RED, mec="white", mew=0.6, color=RED, ecolor=RED, elinewidth=0.9)
    x.axvline(JSTAR[GEOM], color=MUTED, lw=0.8, ls=":")
    x.set_xlim(0.5, 8); x.set_xlabel(r"coupling $J$ [$k_BT$]"); x.set_ylabel(yl)
    x.text(0.04, 0.94, f"({'efg'[c]})", transform=x.transAxes, va="top", fontsize=9, color=INK)
ax[1, 3].axis("off")
ax[1, 3].text(0.0, 0.95, f"{GEOM}: {nfin} / {ntot} runs finished\nJ* (exact, N = 200) = {JSTAR[GEOM]}\n"
              "table helix_pair_db_" + GEOM + "_win.bin\nk = 2K bonded, pairs from 1-4", transform=ax[1, 3].transAxes, va="top", fontsize=9)
fig.tight_layout(); fig.savefig("transitions_db.png", dpi=170); fig.savefig("transitions_db.pdf"); plt.close(fig)

print(f"{GEOM} table scan: {nfin} / {ntot} runs finished")
print(f"{'J':>5} {'n':>2} {'theta MC':>9} {'exact':>7} {'<|m|> MC':>9} {'exact':>7} {'U4 MC':>7} {'exact':>7} {'Rg2':>8} {'E_nb':>7} {'E_twist':>8}")
for J in Js:
