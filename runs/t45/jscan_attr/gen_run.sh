#!/bin/bash
# t45: a WEAK attraction between same-handed helices, opposite-handed pairs still purely repulsive (per user 2026-09-17).
# In this model that is the chiral amplitude just above the point where the same-handed pair energy first turns negative
# (clamped potential, default range sigma_c = 0.51 a).  Deepest same-handed well, always at r = 1.5 a and one sign of psi:
#     eps_s in {8 8.5 9.2}  ->  about {-0.2, -0.5, -1.0} kT;   R.L deepest energy stays > 0 for all of them.
# NOTE: this is beyond the measured interaction (eps_s = 1): a model study of what a same-handed attraction would do.
#   J in {3.5 4.5 5 5.5} (helix-coil midpoint / handedness crossover), 3 seeds.  Reference: the eps_s = 0 runs of ../jscan_*.
cd "$(dirname "$0")"
BIN=${BIN:-../../../zimm}
JS=${JS:-"3.5 4.5 5 5.5"}; ESS=${ESS:-"8 8.5 9.2"}; SEEDS=${SEEDS:-"1 2 3"}; NEQ=${NEQ:-100000}; NSW=${NSW:-1000000}
...
b=J${J}_es${ES}_sc0.51_s${s}; [ -f inputs/$b.dat ] && continue
sed -e "s/^J0 .*/J0          = $J/" -e "s/^J1 .*/J1          = 0/" -e "s/^J2 .*/J2          = $J/" -e 's/^nb_hh.*/nb_hh = fit/' \
    ...
    -e "s/^seed .*/seed        = $((5000000 + n))/" -e "s#^out_prefix .*#out_prefix  = out/$b#" ../sample_data.dat > inputs/$b.dat
printf "E_helix = 0\nhf_theta0 = 45\nhf_len = 1.0\nhf_clamp = 1\nhf_eps_s = $ES\nhf_sig_c = 0.51\nn_flip = 4\n" >> inputs/$b.dat
done; done; done
