...
#   J in {2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5};  3 seeds                                                     (240 runs)
...
ess_of () { case $1 in 0.51) echo "8 8.5 9.2 10.7 12.2";; 0.62) echo "8.4 8.85 9.6 11.2 12.75";; esac; }
...
[ -n "$GEN_ONLY" ] && exit 0
ls inputs/J*.dat | LC_ALL=C sort -t_ -k4,4 -k1.9,1nr -k2.3,2nr | xargs -P ${NPROC:-16} -I{} sh -c 'b=$(basename {} .dat); grep -q summary logs/$b.log 2>/dev/null && exit 0; mkdir logs/$b.lock 2>/dev/null || exit 0; '"$BIN"' {} > logs/$b.log 2>&1; rmdir logs/$b.lock'
