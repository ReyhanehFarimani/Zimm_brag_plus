...
CJ = dict(zip([1, 2, 3, 4, 5, 6], ["#52c79e", "#22b07f", "#149166", "#0e7150", "#09523a", "#043325"]))   # validated ordinal ramp
...
def load(folder, pattern, rx, sc_default=None):
    ...
    for log in sorted(glob.glob(os.path.join(folder, "logs", pattern))):
        ...
        g = re.search(rx, os.path.basename(log)).groups(); J, es = int(g[0]), float(g[1]); sc = float(g[2]) if sc_default is None else sc_default
...
for r, (k, yl, name) in enumerate(KEYS):
    ...
    for c, sc in enumerate(SCs):
        ...
        for n, J in enumerate(Js):
            ...
            x.errorbar(np.array(ess) + (n - 2.5) * 0.06, rel, yerr=rerr, fmt="o-", ms=4.5, lw=1.2, color=CJ[J], mfc=CJ[J], mec=SURF, mew=0.5, elinewidth=0.9, label=f"J = {J}")
...
for k, _, name in KEYS:
    ...
    for sc in SCs:
        ...
        for J in Js:
            ...
            print(f"    J = {J}: " + "   ".join(f"eps {e:g}: {a:+6.3f}+-{b:5.3f} [{a / b:+4.1f}] n={n_}" for e, a, b, n_ in zip(ess, rel, rerr, Ns)))
