#!/bin/bash
# t100: amplify the WHOLE measured helix-helix potential, achiral and chiral sector by the same factor S, per user 2026-09-17:
#   U = S * ( A Ia + C Ic )        ->  hf_scale = S, hf_eps_s = 1  (as measured shape, default range sigma_c = 0.70 a)
# to be compared at the same J with the chiral-only amplification U = A Ia + S * C Ic (hf_scale = 1, hf_eps_s = S) of the
# eps_s scans, and with eps_s = 0.  S in {4 8} are values that exist in the eps_s scan at this range.  Because both
# sectors grow together the potential keeps its shape and stays purely repulsive for every S.
#   J in {2 4 4.5 5}: the helix-coil midpoint and the handedness crossover at N = 200;  3 seeds.
# Bonds/bends from ../sample_data.dat; all lengths in a (hf_len = 1).
cd "$(dirname "$0")"
BIN=${BIN:-../../../zimm}
JS=${JS:-"2 4 4.5 5"}; SCALES=${SCALES:-"4 8"}; SEEDS=${SEEDS:-"1 2 3"}; NEQ=${NEQ:-100000}; NSW=${NSW:-1000000}
...
sed -e "s/^J0 .*/J0          = $J/" -e "s/^J1 .*/J1          = 0/" -e "s/^J2 .*/J2          = $J/" \
    ...
    -e "s/^seed .*/seed        = $((1300000 + n))/" -e "s#^out_prefix .*#out_prefix  = out/$b#" ../sample_data.dat > inputs/$b.dat
printf "E_helix = 0\nhf_theta0 = 100\nhf_len = 1.0\nhf_clamp = 1\nhf_eps_s = 1\nhf_scale = $S\nn_flip = 4\n" >> inputs/$b.dat
done; done; done
