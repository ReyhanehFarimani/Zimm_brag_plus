...
bool assign(Input& in, const std::string& key, const std::string& val) {
    ...
    if (key == "twist_alpha0_rl") return parse(val, in.twist_alpha0_rl);
    if (key == "twist_kappa_rl")  return parse(val, in.twist_kappa_rl);
...
bool read_input(const std::string& filename, Input& in) {
    ...
    if (in.nb_hh == "fit" && in.hf_sig_c < 0.0) {
        std::fprintf(stderr, "input: hf_sig_c must be >= 0 (0 = campaign median)\n"); ok = false;
    }
...
void print_input(const Input& in) {
    ...
    if (in.nb_hh == "db")
        std::printf("# db_file     = %s  (theta0 %d, hf_len %g); n_twist = %d, twist_step = %g deg; "
                    "twist term: alpha0 = %g deg (R.R), kappa = %g kT/rad^2; R.L: alpha0 = %g deg, kappa = %g\n",
    ...
    if (in.nb_hh == "fit")
        std::printf("# hf          = theta0 %d, scale %g, eps_s %g, len %g (code units per a), clamp %d, sig_c %g a%s\n",
