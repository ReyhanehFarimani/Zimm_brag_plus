...
void Logger::header() {
    std::fprintf(obs_, "# %10s %6s %6s %10s %12s %12s %12s %10s %10s\n",
                 "sweep", "n_R", "n_L", "helicity", "E_bond", "Ree", "Rg2", "acc_state", "acc_pos");
    std::fflush(obs_);
}
...
void Logger::sample(long sweep, const Chain& chain, const MC& mc) {
    ...
    std::fprintf(obs_, "  %10ld %6d %6d %10.6f %12.6f %12.6f %12.6f %10.4f %10.4f\n",
                 sweep, nR, nL, th, eb, ree, rg2, mc.acc_state(), mc.acc_pos());
